...
class ClassContainer extends Component
{
    ...
    protected $rules = [
        'class.number' => ['required', 'integer', 'min:0', 'max:999'],
        'class.letter' => ['required', 'string', 'max:10'],
    ...
    protected $messages = [
        ...
        'class.number.required' => 'Поле «номер» является обязательным.',
        'class.number.max' => 'Поле «номер» не может быть числом больше 999.',
        ...
        'class.profile_id.required' => 'Поле «профиль класса» является обязательным.',
        'class.profile_id.exists' => 'Выбранное значение в поле «профиль класса» не найдено.',
        'class.arbitrary_profile_title.max' => 'Поле «профиль класса» не может быть длиннее 191 символов.',
    ];
    ...
    public function updateOrCreateClass()
    {
        ...
        if($query->get()->count()) {
            $this->addError('class.letter', 'Такой класс уже существует');
            return;
        }
    ...
    public function deleteClass()
    {
        try {
            ...
            $this->dispatchBrowserEvent('show-notification', [
                'message' => __('У класса есть руководители и (или) ученики'),
                'type' => 'error',
            ]);
        }
    }
