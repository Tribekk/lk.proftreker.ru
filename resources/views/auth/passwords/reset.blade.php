...
@section('content')
    <div>
        <form class="form" method="POST" action="{{ route('password.update') }}">
            ...
            <x-alert type="info" :close="false" text="На введенный Вами номер телефона отправлен код для подтверждения смены пароля"></x-alert>

            @if(session()->has('sms_code'))
                <x-alert type="info" text="{{ __('Код из смс: ' . session('sms_code')) }}" :close="false"></x-alert>
            @endif
