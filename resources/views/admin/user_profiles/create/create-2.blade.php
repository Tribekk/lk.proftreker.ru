...
<x-inputs.button-link type="btn-outline-success" link="{{ route('products.store') }}" title="{{ __('Перейти к выбору городов и школ') }}"/>
...
<x-inputs.button-link type="btn-outline-success" link="{{ route('products.store') }}" title="{{ __('Перейти к выбору школ') }}"/>
...
<x-tables.filter-inputs.select2
                                ...
                                multiple
                                autofocus
                                label="{{ __('Название школы') }}"
                                placeholder="{{ __('Выбраны все школы') }}"
                                event="filterByParent"
                                indexUrl="{{ route('admin.schools.view') }}"
                                showUrl="/admin/schools"
                                value="{{ request()->title}}">
</x-tables.filter-inputs.select2>
<x-inputs.button-link type="btn-outline-success" link="{{ route('products.store') }}" title="{{ __('Добавить школу') }}"/>
