    ...
    <div class="d-flex flex-column-fluid">
        <div class="container">
            ...
            @forelse($classes as $class)
                            ...
                            <div class="flex-grow-1">
                                <div class="d-flex align-items-center justify-content-between flex-wrap mt-2">
                                    <a href="{{ route('school.classes.show.table', ['class' => $class->id]) }}" class="d-flex link align-items-center text-primary text-hover-danger text-hover-primary font-size-h2 font-weight-bold mr-3">
                                        {{ $class->school->short_title ?? '-' }}. Класс {{ $class->number }}{{ $class->letter }} (год: {{ $class->year ? $class->year : 'не указан' }}, кол-во учащихся: {{ $class->students_count ?? 'не указано' }})
                                    </a>
                                </div>
                ...
                @empty
                <x-alert type="info" text="На данный момент нет классов" :close="false"></x-alert>
            @endforelse
